...
void sort(ChainU <cls> &op) {
    if (op.size() == 0 || op.size() == 1) return;
    NodeU_Iter <cls> zone_pointer{op.head()};
    NodeU_Iter <cls> iterator{op.head()};
...
bool control = false;
for (; main_branch_iterator != op.

tail_c();

++main_branch_iterator) {
if (!control) {
li.link(*new NodeU <cls>{
main_branch_iterator.
...
return std::pair<ChainU < cls>, ChainU <cls>>{
left, right};
}
...
build_iterator.
link(*NodeU<cls>{ri.value_c(), build_iterator.next_ptr()}
);
++
ri;
} else {
build_iterator.
link(*NodeU<cls>{li.value_c(), build_iterator.next_ptr()}
);
++
li;
}
...
void pop_min_circle(ChainUC <cls> &&op) {
    NodeU_Iter <cls> oi{op.head()};
    NodeU_Iter <cls> di{op.head()};
    cls v{op.head()->next_ptr()->value_c()};
    NodeU<cls> * ptr;
...
// 20
template<typename Freq_Node_Iter, typename Freq_Chain, typename cls, typename Freq_Node>
...
// 22
template<typename cls>
NodeU <cls> &common_begin(ChainUC < cls >
const &lhv,
ChainUC <cls> const &rhv
) {
