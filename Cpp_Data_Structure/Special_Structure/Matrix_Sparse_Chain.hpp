...
Matrix_SC<cls> Matrix_SC<cls>::operator+(const Matrix_SC<cls> &op) const {
    ...
    for(;oi!=op.Container.head_c()&&ti!=this->Container.head_c();++ri) {
        if (ti.value_c().first<oi.value_c().first) {
            ri.link(*new NodeU<std::pair<size_t, cls>>{ti.value_c(), ri.next_ptr()});
            ++ti;
        } else if (ti.value_c().first>oi.value_c().first) {
            ri.link(*new NodeU<std::pair<size_t, cls>>{oi.value_c(), ri.next_ptr()});
            ++oi;
        } else {
            ri.link(*new NodeU<std::pair<size_t, cls>>{std::pair<size_t, cls>(oi.value_c().first, oi.value_c().second+ti.value_c().second), ri.next_ptr()});
            ++oi;
            ++ti;
        }
    }
...
Matrix_SC<cls> &Matrix_SC<cls>::operator+=(const Matrix_SC<cls> &op) {
    ...
    for(;oi!=op.Container.head_c()&&ti!=this->Container.head_c();++ti) {
        if (ti.value_c().first>oi.value_c().first) {
            ...
            ti.next().link(*new NodeU<std::pair<size_t, cls>>{std::make_pair(oi.value_c().first, oi.value_c().second+ti.value_c().second), ti.next_ptr()->next_ptr()});
...
Matrix_SC<cls> Matrix_SC<cls>::operator-(const Matrix_SC<cls> &op) const {
    ...
    for(;oi!=op.Container.head_c()&&ti!=this->Container.head_c();++ri) {
        if (ti.value_c().first<oi.value_c().first) {
            ...
            ri.link(*new NodeU<std::pair<size_t, cls>>{std::pair<size_t, cls>(oi.value_c().first, ti.value_c().second-oi.value_c().second), ri.next_ptr()});
            ++oi;
            ++ti;
        }
    }
...
Matrix_SC<cls> &Matrix_SC<cls>::operator-=(const Matrix_SC<cls> &op) {
    ...
    for(;oi!=op.Container.head_c()&&ti!=this->Container.head_c();++ti) {
        if (ti.value_c().first>oi.value_c().first) {
            ...
            ti.next().link(*new NodeU<std::pair<size_t, cls>>{std::make_pair(oi.value_c().first, ti.value_c().second-oi.value_c().second), ti.next_ptr()->next_ptr()});
            ti.link(ti.next().next());
        }
    }
    if (oi!=op.Container.head_c()) {
...
Matrix_SC<cls> Matrix_SC<cls>::operator*(const cls &value) const {
    ...
    for(;ti!=this->Container.head_c();++ri, ++ti) {
        ri.link(*new NodeU<std::pair<size_t, cls>>{std::make_pair(ti.value_c().first, ti.value_c().second*value), ri.next_ptr()});
    }
    return res;
}
...
Matrix_SC<cls> &Matrix_SC<cls>::operator*=(const cls &value) {
    ...
    for(;ti!=this->Container.head_c();++ti) {
        ti.next().link(*new NodeU<std::pair<size_t, cls>>{std::make_pair(ti.value_c().first, ti.value_c()*value.second), ti.next_ptr()->next_ptr()});
        ti.link(ti.next().next());
    }
    return *this;
}
