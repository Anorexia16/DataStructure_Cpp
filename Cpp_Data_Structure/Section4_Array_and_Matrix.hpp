...
Matrix_STL2<bool> make_matrix_from_map(std::map<cls, Cont<cls>> const &map) {
    ...
    for (auto fi = map.begin(); fi != map.end(); ++fi) {
        if (indices_map.template count(fi->first) == 0) {
            indices_map[fi->first] = index;
            ++index;
        }
        for (auto &&si:fi->second) {
            ...
            matrix[indices_map[fi->first]][indices_map[si]] = 1;
        }
    }
    return matrix;
}
