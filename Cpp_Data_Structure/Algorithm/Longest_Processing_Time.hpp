...
{
    Leftist_Tree<Machine_Info> _machine_time = make_LT<Machine_Info>(LT_Index::Height, true);
    ...
    Leftist_Tree<Pair_Unit<double, size_t>> _longest_task = make_LT<Pair_Unit<double, size_t>>(_task_info, LT_Index::Height, false);
    std::vector<std::vector<size_t>> res {};

    for (size_t _index = 0; _index != machine_num; ++_index) res.template emplace_back(std::vector<size_t>{});
    Binary_Node<Machine_Info> _instance = Binary_Node<Machine_Info> {};

    for(size_t _index = 0; _index!=time_chart.size() ; ++_index)
    {
        res[_machine_time.top().Value].emplace_back(_longest_task.top().Value);
        _instance = Binary_Node<Machine_Info>{*_machine_time.Top};
        _instance.value().Key += _longest_task.top().Key;
        _instance.sink();

        _machine_time.insert(*_instance);
        _machine_time.pop();
        _longest_task.pop();
    }
    return res;
