        ...
        if (!_iter->Is_index) {
            ...
            for (size_t _i = 0; _i < _iter->Size - 2; ++_iter)
            {
                if (DC->Keys[_i] == key) {
                    ...
                    DC->Keys[_i + 1] = key;
                    ++_iter->Size;
                    if (_iter->Size > order) this->spilt(*_iter, 1);
                    return;
                }
            }
        } else {
...
void B_Plus_Tree<K_Tp, V_Tp, order>::spilt(B_Plus_Tree::Node &node, const size_t &condition)
{
    if (!node.Is_index)
    {
        ...
        if(condition == 0)
        {
            if (DC->Left_Brother!=nullptr && ((Node*)DC->Left_Brother)->Size != order)
            {
                K_Tp _word = DC->Keys[0];
                DC->Left_Brother->Data[((Node*)DC->Left_Brother)->Size] = DC->Data[0];
                DC->Left_Brother->Keys[((Node*)DC->Left_Brother)->Size] = DC->Keys[0];
                ...
                --((Node*)DC)->Size;
                ++((Node*)DC->Left_Brother)->Size;
                this->reindex(DC->Father, _word, DC->Keys[0]);
            } else {
                _page->Keys[0] = DC->Keys[0];
                _page->Keys[1] = DC->Keys[1];
                _page->Data[0] = DC->Data[0];
                _page->Data[1] = DC->Data[1];

                for (size_t i = 0; i != order - 1; ++i) {
                    DC->Keys[i] = DC->Keys[i + 2];
                    DC->Data[i] = DC->Data[i + 2];
                }
                ...
                if (DC->Father != nullptr) {
                    ...
                    ++_fbp->Size;
                    if (_fbp->Size > order) this->spilt(*_fbp, _fi == 0 ? 0 : 1);
                } else {
            ...
            if (DC->Right_Brother!= nullptr && ((Node*)DC->Right_Brother)->Size!=order)
            {
                for(size_t i=((Node*)DC->Right_Brother)->Size;i>0; --i)
                {
                    DC->Data[i] = DC->Data[i-1];
                    DC->Keys[i] = DC->Keys[i-1];
                }
                DC->Right_Brother->Data[0] = DC->Data[order];
                DC->Right_Brother->Keys[0] = DC->Keys[order];

                --((Node*)DC)->Size;
                ++((Node*)DC->Right_Brother)->Size;
            } else {
                _page->Keys[0] = DC->Keys[order - 1];
                _page->Data[0] = DC->Data[order - 1];
                _page->Keys[1] = DC->Keys[order];
                _page->Data[1] = DC->Data[order];
                static_cast<Node *>(_page)->Size = 2;
                node.Size -= 2;
                ...
                if (DC->Father != nullptr) {
                    ...
                    ++_fbp->Size;
                    if (_fbp->Size > order) this->spilt(*_fbp, _fi == _fbp->Size - 1 ? 2 : 1);
                } else {
                    auto *_rtp = new Index_Node{};
        ...
        if (condition == 0)
        {
            _page->Keys[0] = IC->Keys[0];
            _page->Keys[1] = IC->Keys[1];
            _page->Paths[0] = IC->Paths[0];
            _page->Paths[1] = IC->Paths[1];

            for (size_t i = 0; i != order - 1; ++i) {
                IC->Keys[i] = IC->Keys[i + 2];
                IC->Paths[i] = IC->Paths[i + 2];
            }
            static_cast<Node *>(_page)->Size = 2;
            node.Size -= 2;
            ...
            _page->Paths[1] = IC->Paths[order];
            _page->Keys[1] = IC->Keys[order];
            static_cast<Node*>(_page)->Size = 2;
            node.Size -= 2;
...
template<typename K_Tp, typename V_Tp, size_t order>
void B_Plus_Tree<K_Tp, V_Tp, order>::destroy(B_Plus_Tree::Node &, const size_t &)
{
