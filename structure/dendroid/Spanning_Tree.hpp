...
#include "Reticular/Adjacent_Matrix.hpp"
...
spanning_tree(Adjacent_Matrix<num, true, true> const &matrix)
{
    ...
    Leftist_Tree<KW_Pair<ssize_t, size_t>> _weight_pro {};
    auto &graph = matrix.Container;
    bool inside[num] {};
    size_t index = 0;
    ...
    for(size_t i=0; i!=num; ++i) {
        if(graph[0][i]!=0) {
            _weight_pro.template emplace(KW_Pair<ssize_t, size_t>{-static_cast<ssize_t>(graph[0][i]), index});
        ...
        if (inside[_edge_index]) {
            ...
            for(size_t j=0; j!=num; ++j) {
                if (graph[_edge_index][j] != 0 && !inside[j]) {
                    _weight_pro.template emplace(KW_Pair<ssize_t, size_t>{-static_cast<ssize_t>(graph[_edge_index][j]), index});
